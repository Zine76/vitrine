...
// Démarrer un appel PC (système WebRTC intégré)
async function startPCCall() {
    console.log('📞 Démarrage appel PC avec système WebRTC intégré');
    callMode = 'pc';
    
    try {
        // Créer une room WebRTC
        const roomId = 'UQAM-Salle-' + (getCurrentRoom() || 'PA-204');
        console.log('🏠 Création room WebRTC:', roomId);
        
        // Afficher le modal d'appel PC
        showPCCallModal(roomId);
        
    } catch (error) {
        console.error('❌ Erreur démarrage appel PC:', error);
        alert('Erreur lors du démarrage de l\'appel PC. Vérifiez votre connexion.');
    }
}

// Démarrer un appel téléphone (système WebRTC intégré)
async function startPhoneCall() {
    console.log('📱 Démarrage appel téléphone avec système WebRTC intégré');
    callMode = 'phone';
    ...
    try {
        ...
        alert('Erreur lors du démarrage de l\'appel téléphone. Vérifiez votre connexion.');
    }
}

// Afficher le modal d'appel PC
function showPCCallModal(roomId) {
    // Créer le modal d'appel PC s'il n'existe pas
    let modal = document.getElementById('pcCallModal');
    if (!modal) {
        modal = document.createElement('div');
        modal.id = 'pcCallModal';
        modal.className = 'pc-call-modal-overlay';
        modal.innerHTML = `
            <div class="pc-call-modal">
                <div class="pc-call-modal-header">
                    <h3>💻 Appel PC - Connexion en cours</h3>
                    <button class="pc-call-close-btn" onclick="closePCCallModal()">
                        <i class="fas fa-times"></i>
                    </button>
                </div>
                <div class="pc-call-modal-content">
                    <div class="pc-call-status">
                        <div class="pc-call-spinner">
                            <i class="fas fa-spinner fa-spin"></i>
                        </div>
                        <p>Connexion à la room: <strong>${roomId}</strong></p>
                        <p class="pc-call-info">L'appel va démarrer automatiquement avec les périphériques audio par défaut.</p>
                    </div>
                    <div class="pc-call-actions">
                        <button class="btn btn-secondary" onclick="closePCCallModal()">
                            <i class="fas fa-times"></i> Annuler
                        </button>
                        <button class="btn btn-primary" onclick="endPCCall()" style="display: none;" id="endCallBtn">
    ...
    // Simuler la connexion (dans un vrai système, ceci serait remplacé par la logique WebRTC)
    setTimeout(() => {
        const statusElement = modal.querySelector('.pc-call-status p');
        if (statusElement) {
            statusElement.innerHTML = '✅ Connexion établie ! L\'appel est en cours...';
        }
        
        // Afficher le bouton "Terminer l'appel" après connexion
        setTimeout(() => {
...
// Fermer le modal d'appel PC
function closePCCallModal() {
...
function endPCCall() {
    closePCCallModal();
    showNotification('Appel PC terminé. Dans un vrai système, la connexion WebRTC serait fermée.');
    callMode = 'idle';
}
...
function showPhoneCallStatus() {
    ...
    statusDiv.innerHTML = `
        ...
        <span>Appel téléphone en cours - Audio salle désactivé</span>
    `;
