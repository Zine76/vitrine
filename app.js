...
async function startPCCall() {
    ...
    try {
        ...
        // Utiliser le système WebRTC intégré
        if (typeof CFG !== 'undefined' && CFG.API_BASE) {
            console.log('✅ Utilisation du système WebRTC intégré');
            
            // Créer une connexion WebRTC réelle
            try {
                ...
                if (response.ok) {
                    console.log('✅ Appel PC initié - SAVQonnect devrait sonner');
                ...
                alert('Erreur lors de l\'initiation de l\'appel PC.');
            }
        } else {
            console.log('⚠️ Configuration WebRTC non disponible');
            alert('Configuration WebRTC non disponible. Vérifiez la connexion au serveur.');
        }
        
    } catch (error) {
        console.error('❌ Erreur démarrage appel PC:', error);
        alert('Erreur lors du démarrage de l\'appel PC.');
    }
}
...
async function startPhoneCall() {
    ...
    try {
        ...
        // Générer l'URL Retool pour le téléphone
        phoneJoinURL = `https://retool.dev.uqam.ca/phone/phone-join.html?roomId=${roomId}&salle=${getCurrentRoom() || 'PA-204'}`;
        
        // Définir le temps d'expiration (15 minutes)
        const expiryTime = new Date();
        expiryTime.setMinutes(expiryTime.getMinutes() + 15);
        qrExpiryTime = expiryTime.toLocaleTimeString('fr-FR', { 
            hour: '2-digit', 
            minute: '2-digit' 
        });
