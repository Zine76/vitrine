...
// Démarrer un appel PC (utilise le système existant)
function startPCCall() {
    console.log('📞 Démarrage appel PC');
    callMode = 'pc';
    
    // Utiliser le système d'appel existant
    if (typeof handleCallButtonClick === 'function') {
        handleCallButtonClick();
    } else {
        console.warn('Système d\'appel PC non disponible');
        alert('Système d\'appel PC non disponible. Vérifiez la connexion.');
    }
}

// Démarrer un appel téléphone (nouveau système)
async function startPhoneCall() {
    console.log('📱 Démarrage appel téléphone');
    
    try {
        // Utiliser l'API dynamique (détection automatique du backend)
        const apiBase = await getCurrentAPI();
        
        // Créer une room via l'API
        const response = await fetch(`${apiBase}/rtc/room`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json'
            },
            body: JSON.stringify({ ttl: 15 }) // 15 minutes
        });
        ...
        // 🔒 SÉCURITÉ : URL directe pour le QR code (solution simple)
        // TEMPORAIRE : Utiliser l'URL GitHub Pages pour le déploiement statique
        phoneJoinURL = `https://retool.dev.uqam.ca/phone/phone-join.html?roomId=${roomData.roomId}`;
        
        // URL complète pour le backend (stockée séparément)
        const fullJoinURL = `${apiBase}${roomData.joinURLPhone}`;
        
        // Calculer le temps d'expiration
        const expiryDate = new Date(roomData.expiresAt);
        const now = new Date();
        const diffMinutes = Math.ceil((expiryDate - now) / (1000 * 60));
        qrExpiryTime = `${diffMinutes} minutes`;
        
        // Muter l'audio de la salle (si disponible)
        muteVitrineAudio();
        ...
        console.error('Erreur lors du démarrage de l\'appel téléphone:', error);
