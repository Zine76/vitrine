            ...
            eventSource.onmessage = function(event) {
                try {
                    ...
                    switch (data.type) {
                        ...
                        case 'webrtc_answer':
                            // Réponse WebRTC reçue de SAVQonnect
                            console.log('📞 [WebRTC] Réponse reçue via SSE:', data);
                        ...
                        case 'webrtc_candidate':
                            // Candidat ICE reçu de SAVQonnect
                            console.log('📞 [WebRTC] Candidat ICE reçu via SSE:', data);
                        ...
                        case 'webrtc_ended':
                            // Appel terminé par l'autre partie
                            console.log('📞 [WebRTC] Appel terminé via SSE');
...
async function startPhoneCall() {
    ...
    try {
        ...
        if (response.ok) {
            ...
            // Créer l'URL de jointure pour le téléphone avec l'URL Retool HTTPS
...
// Gérer la réponse WebRTC reçue via SSE (Vitrine reçoit la réponse de SAVQonnect)
window.handleWebRTCAnswer = async function(sdp) {
    console.log('📞 [WebRTC] Traitement de la réponse reçue via SSE');
    ...
    try {
        ...
        console.log('✅ [WebRTC] Réponse WebRTC appliquée');
        
        // Gérer la réception du stream audio distant
        currentPeerConnection.ontrack = (event) => {
            console.log('🎵 [WebRTC] Stream audio distant reçu');
        ...
        console.error('❌ [WebRTC] Erreur application réponse:', error);
    }
};

// Gérer les candidats ICE reçus via SSE
window.handleWebRTCCandidate = async function(candidateData) {
    console.log('📞 [WebRTC] Traitement candidat ICE reçu via SSE');
    ...
    try {
        // Le candidateData peut être une string JSON ou un objet
        ...
        console.log('✅ [WebRTC] Candidat ICE ajouté');
    } catch (error) {
...
window.cleanupWebRTCCall = function() {
    ...
    console.log('✅ [WebRTC] Appel nettoyé');
};

// Créer un élément audio pour le stream distant si nécessaire
