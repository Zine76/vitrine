...
console.log('?? [Version] app.js LOCAL - Trace modifications réseau localhost v1.0');
        ...
        let API_BASE_URL = (function(){
            ...
            console.log('?? [Config] Aucune IP configurée. Utilisez Alt+Ctrl+J pour configurer le backend.');
            return null;
        })();
        ...
        async function detectBestBackend() {
            ...
            if (!API_BASE_URL) {
                console.log('?? [Config] Aucune IP configurée pour le test');
                return null;
            }
            ...
            try {
                ...
                console.log(`?? [Config] Backend configuré inaccessible: ${API_BASE_URL}`);
            }
        ...
        const backendInitPromise = (async function initializeBackend() {
            try {
                ...
                if (!API_BASE_URL) {
                    console.log('?? [Config] Aucune IP configurée. Affichage du modal de configuration...');
...
// ✅ EXPOSITION DES FONCTIONS VITRINE POUR HTML
// Exposer les fonctions principales pour vitrine.html
if (typeof confirmRoom === 'function') {
    window.confirmRoom = confirmRoom;
}
if (typeof handleRoomKeyPress === 'function') {
    window.handleRoomKeyPress = handleRoomKeyPress;
}
if (typeof setRoomExample === 'function') {
    window.setRoomExample = setRoomExample;
}
if (typeof startPCCall === 'function') {
    window.startPCCall = startPCCall;
}
if (typeof startPhoneCall === 'function') {
    window.startPhoneCall = startPhoneCall;
}
if (typeof changeRoom === 'function') {
    window.changeRoom = changeRoom;
}
if (typeof sendExampleMessage === 'function') {
    window.sendExampleMessage = sendExampleMessage;
}
if (typeof toggleTheme === 'function') {
    window.toggleTheme = toggleTheme;
}
if (typeof openTechnicalMode === 'function') {
    window.openTechnicalMode = openTechnicalMode;
}

console.log('✅ [app.js] Fonctions exposées globalement pour vitrine.html');
...
async function createWebRTCConnection(roomId, callData) {
    try {
        ...
        // Stocker le stream et le roomId
        ...
        // Afficher la bannière d'appel
        showCallBanner(roomId);
...
async function sendWebRTCOffer(offer, roomId) {
    try {
        ...
        if (response.ok) {
            ...
            console.error('Erreur envoi offre WebRTC:', await response.text());
        }
    } catch (error) {
        console.error('Erreur sendWebRTCOffer:', error);
    }
}
...
async function sendIceCandidate(candidate, roomId) {
    try {
        await fetch(`${currentAPI}/api/webrtc/candidate`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
            },
        ...
        console.log('Candidat ICE envoyÃ© au serveur');
    } catch (error) {
...
// ===============================================
// BanniÃ¨res d'appel avec contrÃ´les
// ===============================================

function showCallBanner(roomId) {
    const existingBanner = document.getElementById('webrtc-call-banner');
    if (existingBanner) existingBanner.remove();
    
    const banner = document.createElement('div');
    banner.id = 'webrtc-call-banner';
    banner.className = 'call-banner';
    
    banner.innerHTML = `
        <div class="call-banner-content">
            <div class="call-banner-title">ðŸ“ž Appel en cours</div>
            <div class="call-banner-room" id="call-room-name">${roomId.replace('UQAM-Salle-', 'Salle ')}</div>
            <div class="call-banner-status" id="call-status">En attente de connexion...</div>
            <div class="call-banner-controls">
                <button id="mute-btn" class="call-control-btn mute-btn">
                    <span class="btn-icon">ðŸŽ¤</span>
                    <span class="btn-text">Mute</span>
                </button>
                <button id="hangup-btn" class="call-control-btn hangup-btn">
                    <span class="btn-icon">ðŸ“ž</span>
                    <span class="btn-text">Raccrocher</span>
                </button>
            </div>
        </div>
    `;
...
function updateCallBannerStatus(status) {
    const statusEl = document.getElementById('call-status');
    if (statusEl) {
        if (status === 'connected') {
            statusEl.textContent = 'âœ… ConnectÃ© - Communication active';
            statusEl.style.color = '#4ade80';
        } else if (status === 'connecting') {
            statusEl.textContent = 'ðŸ”„ Connexion en cours...';
            statusEl.style.color = '#fbbf24';
        } else if (status === 'disconnected') {
            statusEl.textContent = 'âŒ DÃ©connectÃ©';
...
function closeCallBanner() {
    const banner = document.getElementById('webrtc-call-banner');
    if (banner) {
        banner.classList.remove('show');
...
function toggleMute() {
    ...
    console.log('ðŸŽ¤ Micro ' + (isAudioMuted ? 'mutÃ©' : 'activÃ©'));
}

function hangupCall() {
    console.log('ðŸ“ž Raccrochage de l\'appel');
    if (window.cleanupWebRTCCall) window.cleanupWebRTCCall();
    if (currentRoomId && currentCallToken) {
        fetch(currentAPI + '/api/webrtc/hangup', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ roomId: currentRoomId, callToken: currentCallToken })
        }).catch(err => console.error('Erreur hangup:', err));
    }
    closeCallBanner();
}

// ✅ EXPOSITION FINALE DES FONCTIONS VITRINE POUR HTML
// S'assurer que les fonctions sont disponibles globalement après tous les wrappers
console.log('🔧 [app.js] Exposition finale des fonctions pour vitrine.html...');
if (typeof confirmRoom === 'function') {
    window.confirmRoom = confirmRoom;
    console.log('✅ [app.js] confirmRoom exposée');
}
if (typeof handleRoomKeyPress === 'function') {
    window.handleRoomKeyPress = handleRoomKeyPress;
    console.log('✅ [app.js] handleRoomKeyPress exposée');
}
if (typeof setRoomExample === 'function') {
    window.setRoomExample = setRoomExample;
    console.log('✅ [app.js] setRoomExample exposée');
}
if (typeof startPCCall === 'function') {
    window.startPCCall = startPCCall;
    console.log('✅ [app.js] startPCCall exposée');
}
if (typeof startPhoneCall === 'function') {
    window.startPhoneCall = startPhoneCall;
    console.log('✅ [app.js] startPhoneCall exposée');
}
if (typeof changeRoom === 'function') {
    window.changeRoom = changeRoom;
    console.log('✅ [app.js] changeRoom exposée');
}
if (typeof sendExampleMessage === 'function') {
    window.sendExampleMessage = sendExampleMessage;
    console.log('✅ [app.js] sendExampleMessage exposée');
}
if (typeof toggleTheme === 'function') {
    window.toggleTheme = toggleTheme;
    console.log('✅ [app.js] toggleTheme exposée');
}
if (typeof openTechnicalMode === 'function') {
    window.openTechnicalMode = openTechnicalMode;
    console.log('✅ [app.js] openTechnicalMode exposée');
}

console.log('🎉 [app.js] Toutes les fonctions exposées globalement pour vitrine.html !');
