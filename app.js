        ...
        (function setupBackendHealthWatch(){
            async function pingOnce(signal){
                // Utiliser le ConnectionManager si disponible
                if (window.connectionManager) {
                    // Simplement retourner l'état actuel du ConnectionManager
                    const isConnected = window.connectionManager.isConnected;
                    
                    // Mettre à jour l'interface utilisateur pour la compatibilité
                    ...
                    if (txt) { txt.textContent = isConnected ? 'Système opérationnel' : 'Reconnexion en cours...'; }
                    
                    return isConnected;
                }
                
                // Fallback vers l'ancienne méthode si ConnectionManager n'est pas disponible
                try {
            ...
            document.addEventListener('DOMContentLoaded', () => {
                // Ne pas démarrer l'intervalle si ConnectionManager est disponible
                if (!window.connectionManager) {
                    pingOnce();
                    // Pings périodiques
                    setInterval(() => pingOnce(), 20000);
                } else {
                    console.log('ℹ️ [BackendHealthWatch] Utilisation du ConnectionManager pour la surveillance');
                }
            });
        })();
        ...
        function restartSSEConnections() {
            ...
            // Utiliser le ConnectionManager si disponible
            if (window.connectionManager) {
                // Le ConnectionManager va gérer automatiquement les reconnexions des EventSource
                if (window.connectionManager.isConnected) {
                    console.log('✅ [SSERestart] Utilisation du ConnectionManager pour les reconnexions');
                    ...
                    console.log('⏳ [SSERestart] Attente de la reconnexion du ConnectionManager avant de redémarrer les SSE');
                    return;
                }
            }
            
            // Méthode originale si ConnectionManager n'est pas disponible
            // Redémarrer Chat SSE
            if (getCurrentRoom()) {
                setTimeout(() => {
                    startChatRequestListener();
