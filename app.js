            ...
            async getRoomInfo(roomName) {
                ...
                try {
                    ...
                    try {
                        ...
                        // ✅ CORRECTION: S'assurer que apiUrl est défini dans le scope du catch
...
// ===== FONCTIONS POUR APPELS PC ET TÉLÉPHONE =====
...
// Démarrer un appel PC (vrai système WebRTC intégré)
async function startPCCall() {
    console.log('📞 Démarrage appel PC avec vrai système WebRTC intégré');
    callMode = 'pc';
    
    try {
        // Créer une room WebRTC réelle
        const roomId = 'UQAM-Salle-' + (getCurrentRoom() || 'PA-204');
        console.log('🏠 Création room WebRTC réelle:', roomId);
        ...
        if (response.ok) {
            ...
            console.log('✅ Appel PC initié - Données reçues:', callData);
            ...
            alert('Appel PC initié ! SAVQonnect devrait sonner.');
        } else {
        ...
        console.error('❌ Erreur démarrage appel PC:', error);
        alert('Erreur lors du démarrage de l\'appel PC: ' + error.message);
    }
}
...
async function createWebRTCConnection(roomId, callData) {
    try {
        console.log('🔗 Création connexion WebRTC pour room:', roomId);
        ...
        currentPeerConnection.onconnectionstatechange = () => {
            console.log('🔗 État connexion WebRTC:', currentPeerConnection.connectionState);
        ...
        console.log('🎤 Stream audio local ajouté à la connexion WebRTC');
...
async function sendWebRTCOffer(offer, roomId) {
    try {
        ...
        if (response.ok) {
            console.log('✅ Offre WebRTC envoyée au serveur');
        } else {
...
// Démarrer un appel téléphone (QR code Retool - indépendant du PC)
async function startPhoneCall() {
    console.log('📱 Démarrage appel téléphone via QR code Retool');
    callMode = 'phone';
    
    try {
        // Créer une room WebRTC réelle
        const roomId = 'UQAM-Salle-' + (getCurrentRoom() || 'PA-204');
        console.log('🏠 Création room WebRTC pour téléphone:', roomId);
        
        // S'assurer de la connexion backend
        await ensureBackendConnection();
        
        // Notifier SAVQonnect de l'appel téléphone
        const response = await fetch(`${currentAPI}/api/webrtc/start-call`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
        ...
        if (response.ok) {
            const callData = await response.json();
            console.log('✅ Appel téléphone initié - Données reçues:', callData);
            
            // Stocker les informations d'appel
            currentCallToken = callData.callToken;
            currentIceServers = callData.iceServers;
            
            // Créer l'URL de jointure pour le téléphone
            const phoneJoinURL = `${currentAPI}/phone-join.html?room=${encodeURIComponent(roomId)}&token=${encodeURIComponent(callData.callToken)}&salle=${encodeURIComponent(getCurrentRoom() || 'PA-204')}`;
            ...
            const errorText = await response.text();
            console.error('❌ Détails erreur:', errorText);
            alert('Erreur serveur lors de l\'initiation de l\'appel téléphone.');
        }
        
    } catch (error) {
        console.error('❌ Erreur démarrage appel téléphone:', error);
        alert('Erreur lors du démarrage de l\'appel téléphone: ' + error.message);
    }
}

// Afficher le QR code pour l'appel téléphone
function showPhoneQRCode(phoneJoinURL, roomId) {
    // Créer une modal pour le QR code
    const modal = document.createElement('div');
    modal.className = 'phone-qr-modal';
    modal.innerHTML = `
        <div class="phone-qr-content">
            <h3>📱 Appel Téléphone</h3>
...
// Générer un QR code (méthode simple avec service en ligne)
...
function closePhoneQR() {
    ...
    window.currentPhoneURL = null;
        ...
        // IMPORTANT: L'appel téléphone via QR code Retool est complètement indépendant
        // Il ne doit PAS toucher à l'audio du PC (micro/haut-parleurs)
        // L'audio se fait uniquement via le téléphone qui scanne le QR code
        console.log('📱 Appel téléphone indépendant - Audio PC non affecté');
        console.log('📱 L\'audio se fait uniquement via le téléphone qui scanne le QR code');
        
        // Notifier SAVQonnect de l'appel téléphone (pour la sonnerie)
        if (typeof CFG !== 'undefined' && CFG.API_BASE) {
            try {
                const response = await fetch(`${CFG.API_BASE}/api/webrtc/start-call`, {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                    },
                ...
                if (response.ok) {
                    console.log('✅ Appel téléphone notifié - SAVQonnect devrait sonner');
                } else {
                    console.error('❌ Erreur serveur téléphone:', response.status);
                }
            } catch (error) {
                console.error('❌ Erreur notification téléphone:', error);
            }
        }
        ...
        console.error('❌ Erreur démarrage appel téléphone:', error);
        alert('Erreur lors du démarrage de l\'appel téléphone.');
    }
}

// Les fonctions de simulation ont été supprimées
// L'ancienne Vitrine redirige maintenant vers le nouveau système WebRTC
...
function showQRModal() {
    ...
    if (modal) {
        ...
        const urlSpan = document.getElementById('phoneJoinURL');
        const expirySpan = document.getElementById('qrExpiryTime');
        
        if (urlSpan) urlSpan.textContent = phoneJoinURL;
        if (expirySpan) expirySpan.textContent = qrExpiryTime;
...
async function copyPhoneURL() {
    try {
        await navigator.clipboard.writeText(phoneJoinURL);
        alert('URL copiée dans le presse-papiers');
    } catch (error) {
        console.error('Erreur lors de la copie:', error);
        // Fallback pour les navigateurs plus anciens
        const textArea = document.createElement('textarea');
        textArea.value = phoneJoinURL;
        document.body.appendChild(textArea);
        textArea.select();
        document.execCommand('copy');
        document.body.removeChild(textArea);
...
async function endPhoneCall() {
    if (currentRoomId) {
        try {
            ...
            // Utiliser l'API dynamique (détection automatique du backend)
            const apiBase = await getCurrentAPI();
            
            // Envoyer un signal de hangup
            await fetch(`${apiBase}/rtc/signal`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json'
                },
...
// Muter l'audio de la salle
function muteVitrineAudio() {
    // Chercher les éléments audio existants
    ...
    console.log('🔇 Audio de la salle muté pour l\'appel téléphone');
}

// Réactiver l'audio de la salle
function unmuteVitrineAudio() {
    const audioElements = document.querySelectorAll('audio');
    audioElements.forEach(audio => {
        audio.muted = false;
    });
    ...
    console.log('🔊 Audio de la salle réactivé');
}

// Afficher le statut d'appel téléphone
function showPhoneCallStatus() {
    // Supprimer le statut existant s'il y en a un
    hidePhoneCallStatus();
    
    const statusDiv = document.createElement('div');
    statusDiv.id = 'phoneCallStatus';
    statusDiv.className = 'phone-call-status';
    statusDiv.innerHTML = `
        <div class="status-indicator"></div>
        <span>Appel téléphone en cours via QR code - Audio PC non affecté</span>
    `;
    
    document.body.appendChild(statusDiv);
}

// Masquer le statut d'appel téléphone
function hidePhoneCallStatus() {
...
async function generateQRCode() {
    ...
    if (!qrContainer || !phoneJoinURL) return;
    
    // Nettoyer le conteneur
    qrContainer.innerHTML = '';
    
    try {
        // Essayer d'utiliser une librairie QR code en ligne
        const qrApiUrl = `https://api.qrserver.com/v1/create-qr-code/?size=200x200&data=${encodeURIComponent(phoneJoinURL)}`;
        
        const img = document.createElement('img');
        img.src = qrApiUrl;
        img.alt = 'QR Code pour appel téléphone';
        img.style.cssText = `
            width: 200px;
            height: 200px;
            border-radius: 8px;
        ...
        fallbackDiv.innerHTML = `
            ...
            <div style="font-size: 10px; margin-top: 5px; word-break: break-all; padding: 0 10px;">${phoneJoinURL}</div>
        `;
...
window.addEventListener('beforeunload', () => {
    if (callMode === 'phone' && currentRoomId) {
        // Envoyer un signal de hangup avant de fermer
        navigator.sendBeacon('/rtc/signal', JSON.stringify({
            roomId: currentRoomId,
            role: 'pc',
            type: 'hangup',
            data: null
