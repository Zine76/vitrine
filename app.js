...
/* === FLEX CHAT INPUT (auto-grow + reset) === */
(function(){
  function setupFlexibleChatInput(){
    ...
    if(el.tagName === 'INPUT'){
      ...
      for (var i=0; i<el.attributes.length; i++){
        ...
        if(a && a.name && a.value != null) ta.setAttribute(a.name, a.value);
      }
    ...
    window.resetChatInput = function(){
  ...
  document.addEventListener('DOMContentLoaded', setupFlexibleChatInput);
  var modal = document.getElementById('chatModal');
  if (modal){
    var obs = new MutationObserver(function(){
      if (modal.classList.contains('active')) setupFlexibleChatInput();
    });
    obs.observe(modal, {attributes:true, attributeFilter:['class']});
  }
  window.setupFlexibleChatInput = setupFlexibleChatInput;
})();


/* === FLEX CHAT INPUT hook after send === */
(function(){
  ...
  function install(){
    ...
    try{
      if (typeof window.sendChatMessage === 'function'){
        var __orig = window.sendChatMessage;
        window.sendChatMessage = async function(){
          var r;
          try{ r = __orig.apply(this, arguments); if (r && typeof r.then==='function') await r; }
          finally { if (typeof window.resetChatInput==='function') window.resetChatInput(); }
          return r;
        };
        _installed = true;
      }
    }catch(e){ console.warn('[FlexInput] hook sendChatMessage failed', e); }
  }
  install();
