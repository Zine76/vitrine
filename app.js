...
// Démarrer un appel PC (nouveau système WebRTC)
async function startPCCall() {
    console.log('📞 Démarrage appel PC avec nouveau système WebRTC');
    callMode = 'pc';
    
    try {
        // Rediriger vers le nouveau système WebRTC
        let newVitrineUrl;
        if (window.location.protocol === 'file:') {
            // Pour les URLs file://, construire le chemin relatif correctement
            const currentPath = window.location.pathname;
            const basePath = currentPath.substring(0, currentPath.indexOf('/Annexe/Vitrine-Github-version'));
            newVitrineUrl = window.location.origin + basePath + '/vitrine.html';
        } else {
            // Pour les URLs http/https, utiliser la logique existante
            newVitrineUrl = window.location.origin.replace('/Annexe/Vitrine-Github-version', '') + '/vitrine.html';
        }
        console.log('🔄 Redirection vers le nouveau système:', newVitrineUrl);
        window.location.href = newVitrineUrl;
    } catch (error) {
        console.error('❌ Erreur redirection:', error);
        alert('Erreur lors de l\'ouverture du système d\'appel. Veuillez utiliser la nouvelle version de la Vitrine.');
    }
}

// Démarrer un appel téléphone (nouveau système WebRTC)
async function startPhoneCall() {
    console.log('📱 Démarrage appel téléphone avec nouveau système WebRTC');
    callMode = 'phone';
