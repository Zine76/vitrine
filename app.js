...
// Démarrer un appel PC (vrai système WebRTC)
async function startPCCall() {
    console.log('📞 Démarrage appel PC avec vrai système WebRTC');
    callMode = 'pc';
    
    try {
        // Utiliser le vrai système WebRTC depuis config.js
        if (typeof CFG !== 'undefined' && CFG.startPCCall) {
            await CFG.startPCCall();
        } else {
            // Fallback vers le nouveau système
            const roomId = 'UQAM-Salle-' + (getCurrentRoom() || 'PA-204');
            console.log('🏠 Redirection vers nouveau système WebRTC:', roomId);
            window.location.href = '../vitrine.html';
        }
    } catch (error) {
        console.error('❌ Erreur démarrage appel PC:', error);
        alert('Erreur lors du démarrage de l\'appel PC. Redirection vers le nouveau système...');
        window.location.href = '../vitrine.html';
    }
}

// Démarrer un appel téléphone (vrai système WebRTC)
async function startPhoneCall() {
    console.log('📱 Démarrage appel téléphone avec vrai système WebRTC');
    callMode = 'phone';
    
    try {
        // Utiliser le vrai système WebRTC depuis config.js
        if (typeof CFG !== 'undefined' && CFG.startPhoneCall) {
            await CFG.startPhoneCall();
        } else {
            // Fallback vers le nouveau système
            const roomId = 'UQAM-Salle-' + (getCurrentRoom() || 'PA-204');
            console.log('🏠 Redirection vers nouveau système WebRTC:', roomId);
        ...
        alert('Erreur lors du démarrage de l\'appel téléphone. Redirection vers le nouveau système...');
        window.location.href = '../vitrine.html';
    }
}
