...
function initializeCallSystem() {
    ...
    // Auto-configure default devices if none saved
    autoConfigureDefaultDevices();
...
function initializeUI() {
    ...
    if (!document.querySelector('#waitingAudio')) {
        ...
        // Utiliser un son intégré plus long et agréable pour éviter l'effet staccato
        waitingAudio.src = 'data:audio/wav;base64,UklGRnoGAABXQVZFZm10IBAAAAABAAEAQB8AAEAfAAABAAgAZGF0YQoGAACBhYqFbF1fdJivrJBhNjVgodDbq2EcBj+a2/LDciUFLIHO8tiJNwgZaLvt559NEAxQp+PwtmMcBjiR1/LMeSwFJHfH8N2QQAoUXrTp66hVFApGn+DyvmwhBSl+zPPTgjMGHm7A7+OZURE';
        waitingAudio.loop = true;
        waitingAudio.volume = 0.2; // Volume plus doux
    ...
    if (!document.querySelector('#ringingAudio')) {
        ...
        // Utiliser un son intégré pour la sonnerie (pas de fichier externe)
...
function handleCallButtonClick() {
    ...
    console.log('🔍 [Audio] Clic bouton appel - Périphériques:', {
        mic: micId ? '✅ Configuré' : '❌ Non configuré',
        speaker: speakerId ? '✅ Configuré' : '❌ Non configuré'
    });
    
    // Si au moins le microphone est configuré, on peut démarrer l'appel
    if (micId) {
        console.log('📞 Microphone configuré, démarrage appel direct...');
        startCallDirectly();
    } else {
        console.log('📞 Aucun périphérique configuré, affichage modal...');
        showDeviceModal();
    }
}
...
// Auto-configure default devices if none are saved
async function autoConfigureDefaultDevices() {
    const savedMicId = localStorage.getItem('micDeviceId');
    const savedSpeakerId = localStorage.getItem('speakerDeviceId');
    
    // Si aucun périphérique n'est configuré, configurer automatiquement les défauts
    if (!savedMicId && !savedSpeakerId) {
        try {
            console.log('🔧 [Audio] Configuration automatique des périphériques par défaut...');
            ...
            // Configurer le premier microphone disponible
            if (audioInputs.length > 0) {
                localStorage.setItem('micDeviceId', audioInputs[0].deviceId);
                console.log('✅ [Audio] Microphone par défaut configuré:', audioInputs[0].label);
            }
            
            // Configurer la première sortie audio disponible
            if (audioOutputs.length > 0) {
                localStorage.setItem('speakerDeviceId', audioOutputs[0].deviceId);
                console.log('✅ [Audio] Haut-parleur par défaut configuré:', audioOutputs[0].label);
            }
            ...
            console.error('❌ [Audio] Erreur configuration automatique:', err);
        }
    }
}
